...
export const onboardingTranslations = pgTable('onboarding_translations', {
  id: serial('id').primaryKey(),
  screenId: integer('screen_id').references(() => onboardingScreens.id, { onDelete: 'cascade' }),
  languageId: varchar('language_id', { length: 5 }).references(()=> supportedLanguages.id, { onDelete: 'cascade' }),
  title: text('title'),
