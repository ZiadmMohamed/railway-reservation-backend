...
export class stationRepo{
    ...
    async create(stationArabicName:CreateStationDto["stationArabicName"],stationEnglishName:CreateStationDto["stationEnglishName"]){
    const created = await this.db.insert(stations).values({stationArabicName,stationEnglishName}).returning();
     return created[0]
    }
